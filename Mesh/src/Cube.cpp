...
void Cube::generateVertices(const glm::vec3 &width) {
  ...
  m_vertices.push_back(ShapeVertex(glm::vec3(upper.x, lower.y, lower.z), glm::vec3(upper.x, lower.y, lower.z), glm::vec2(0, 0))); // BOTTOM RIGHT
  m_vertices.push_back(ShapeVertex(glm::vec3(lower.x, lower.y, lower.z), glm::vec3(lower.x, lower.y, lower.z), glm::vec2(1, 0))); // BOTTOM LEFT
...
void Cube::generateIndexs() {
  ...
  m_indexVer.insert(m_indexVer.end(),{1, 2, 5, 2, 5, 6}); // BOTTOM FACE
}